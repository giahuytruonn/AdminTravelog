    ...
    const columns: ColumnsType<ExploreVideo> = [
        {
            ...
            width: 150,
            render: (link) => (
                <div style={{ position: 'relative', width: 120, height: 70, background: '#000', borderRadius: 4, overflow: 'hidden', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                     <video 
                        src={link} 
                        ...
                        muted
                    />
                    <PlayCircleOutlined style={{ position: 'absolute', fontSize: 24, color: 'rgba(255,255,255,0.8)' }} />
                </div>
            )
        },
        {
        ...
        },
        {
            ...
            key: 'tourID',
            width: 120,
            render: (tourID) => tourID ? <Tag color="blue">Linked</Tag> : <Tag color="orange">General Promo</Tag>
        },
        {
            title: 'Likes',
            dataIndex: 'likes',
        ...
        // Cột Created At nếu cần thiết
        {
            ...
            key: 'createdAt',
            width: 120,
            render: (date) => date?.seconds ? new Date(date.seconds * 1000).toLocaleDateString('vi-VN') : '-',
        }
    ];
